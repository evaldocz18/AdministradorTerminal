...
public class CadastrarDispostivoKiosqueActivity extends AppCompatActivity {
    ...
    public void clickAtivarDispositivo(View view) {
        if  (idDispositivoKiosque.getText().toString().equals("") ){
            ...
            DispositivosKiosqueReferencia.child("idDispositivo").child(pegarIDDispositivo()).child("status").setValue(statusDispositivoKiosque.getSelectedItem().toString());
            DispositivosKiosqueReferencia.child("idDispositivo").child(pegarIDDispositivo()).child("questionarioAtual").setValue("SEM QUESTIONARIO");
